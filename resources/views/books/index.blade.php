...
<div class="bg-white rounded-2xl border border-sky-100 shadow-sm overflow-hidden">
    ...
    <div class="px-6 py-4 border-b border-sky-50 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div class="flex items-center gap-3">
            ...
            <span class="bg-sky-50 text-sky-700 text-xs font-semibold px-2.5 py-1 rounded-md border border-sky-100">{{ $books->total() }} Total Buku</span>
        </div>
        ...
        <form action="{{ route('books.index') }}" method="GET" class="w-full md:w-auto flex flex-col md:flex-row items-center gap-2">
            <!-- Filter Kategori -->
            <select name="category" onchange="this.form.submit()" class="w-full md:w-auto block px-3 py-2 border border-sky-200 rounded-xl text-sm text-slate-700 focus:outline-none focus:ring-1 focus:ring-sky-500 focus:border-sky-500 bg-white transition-colors cursor-pointer">
            ...
            <div class="relative w-full md:w-64">
                ...
                <input type="text" name="search" value="{{ request('search') }}" class="block w-full pl-10 pr-3 py-2 border border-sky-200 rounded-xl text-sm placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-sky-500 focus:border-sky-500 transition-colors" placeholder="Cari judul...">
            </div>
            
            @if(request('search') || request('category'))
                <a href="{{ route('books.index') }}" class="p-2 text-slate-400 hover:text-sky-500 transition-colors" title="Hapus Filter">
            ...
            <tbody class="divide-y divide-sky-50 text-sm whitespace-nowrap">
                ...
                <tr class="hover:bg-sky-50/30 transition-colors">
                    <td class="px-4 py-3">
                        <div class="flex items-center gap-3">
                            @if($book->image)
                                <img src="{{ asset('storage/' . $book->image) }}" alt="Cover" class="w-10 h-12 object-cover rounded shadow-sm border border-slate-200">
                            @else
                                <div class="w-10 h-12 bg-slate-100 rounded flex items-center justify-center border border-slate-200 text-slate-400">
                    ...
                    <td class="px-4 py-3 text-center">
                        ...
                        @else
                            <span class="inline-flex items-center gap-1.5 px-2 py-1 rounded-md text-[11px] font-bold bg-sky-50 text-sky-700 border border-sky-100">
                                <span class="w-1.5 h-1.5 rounded-full bg-gradient-to-r from-sky-500 to-sky-200 shadow-[0_0_4px_rgba(244,63,94,0.5)]"></span>
                                Habis
                            </span>
                        @endif
                    </td>
                    <td class="px-4 py-3 text-right">
                        <div class="flex justify-end gap-2">
                            <a href="{{ route('books.show', $book->id) }}" class="p-2 text-slate-500 hover:text-sky-600 hover:bg-sky-50 rounded-md transition-colors border border-transparent hover:border-sky-100" title="Detail">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                  <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                  <path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                </svg>
                            </a>
                            <a href="{{ route('books.edit', $book->id) }}" class="p-2 text-sky-600 hover:bg-sky-50 rounded-md transition-colors border border-transparent hover:border-sky-100" title="Edit">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <tr>
                    <td colspan="6" class="px-6 py-16 text-center">
                        ...
                        @if(!(request('search') || request('category')))
                        <a href="{{ route('books.create') }}" class="inline-flex items-center text-sm font-medium text-sky-600 hover:text-sky-700">
                            Tambah Buku Pertama
                        ...
                        @else
                        <a href="{{ route('books.index') }}" class="inline-flex items-center text-sm font-medium text-sky-600 hover:text-sky-700">
