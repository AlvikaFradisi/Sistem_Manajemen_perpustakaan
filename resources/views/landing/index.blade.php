...
@section('content')
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
        <div class="bg-white rounded-3xl p-6 border border-sky-100 shadow-sm flex items-start gap-4 hover:shadow-md transition-shadow">
    ...
    <div id="katalog" class="scroll-mt-32">
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
            ...
            <form action="{{ route('landing.index') }}#katalog" method="GET" class="w-full md:w-auto flex flex-col sm:flex-row items-center gap-3">
                <select name="category" onchange="this.form.submit()" class="w-full sm:w-auto px-4 py-3 bg-white border border-sky-200 rounded-xl text-sm font-medium text-slate-700 focus:ring-2 focus:ring-sky-500 focus:border-sky-500 cursor-pointer shadow-sm">
        ...
        <div class="mb-6 flex items-center justify-between bg-white px-5 py-3 rounded-xl border border-sky-100 shadow-sm">
            <p class="text-sm font-medium text-slate-600">
                ...
                @if(request('category')) kategori <span class="font-bold text-sky-600">"{{ request('category') }}"</span> @endif
            </p>
            <a href="{{ route('landing.index') }}#katalog" class="text-sm font-semibold text-sky-500 hover:text-sky-700">Reset Filter</a>
        </div>
        @endif
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-12">
            ...
            <div class="bg-white rounded-2xl border border-sky-100 overflow-hidden hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] hover:-translate-y-1 transition-all duration-300 group flex flex-col h-full">
                <!-- Book Cover/Visual -->
                <div class="h-48 md:h-56 bg-sky-50 relative p-4 flex items-center justify-center overflow-hidden border-b border-sky-100 group-hover:bg-sky-100 transition-colors">
                    <div class="absolute inset-0 bg-gradient-to-t from-slate-900/40 to-transparent z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                    
                    @if($book->image)
                        <img src="{{ asset('storage/' . $book->image) }}" alt="Cover {{ $book->title }}" class="w-full h-full object-cover rounded shadow-sm z-0 relative group-hover:scale-105 transition-transform duration-500">
                    @else
                        <div class="w-24 h-32 bg-white rounded shadow-sm flex items-center justify-center text-sky-200 border border-sky-100 z-0 relative group-hover:scale-105 transition-transform duration-500">
                ...
                <div class="p-5 flex-grow flex flex-col">
                    <div class="mb-auto">
                        <span class="inline-block px-2.5 py-1 mb-3 text-[10px] font-bold uppercase tracking-wider rounded-md bg-sky-50 text-sky-600 border border-sky-100">
                            {{ $book->category ?? 'Umum' }}
                        </span>
                        <h3 class="text-lg font-bold text-slate-800 mb-1 line-clamp-2 leading-tight group-hover:text-sky-600 transition-colors">
                            {{ $book->title }}
                        </h3>
            ...
            <div class="col-span-full py-20 bg-white rounded-3xl border border-sky-100 text-center shadow-sm">
                <div class="inline-flex items-center justify-center w-20 h-20 rounded-full bg-sky-50 mb-6">
                    <svg class="h-10 w-10 text-sky-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                    </svg>
                </div>
